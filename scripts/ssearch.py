...
class Compare:
    ...
    def water(self, subject, target):
        ...
        shuffled_target_file = tempfile.NamedTemporaryFile(mode="w+b")
        ...
        for i in range(self.shuffle):
            ...
            record = SeqRecord(Seq(''.join(random),IUPAC.protein),name=target.name,id=target.name,description=target.name)
            SeqIO.write(record,shuffled_target_file,'fasta')
        # Write Subject FASTA to temporary file
        subject_file = tempfile.NamedTemporaryFile(mode="w+b")
        record = SeqRecord(Seq(''.join(subject.seq),IUPAC.protein),name=subject.name,id=subject.name,description=subject.name)
        ...
        command = 'ssearch36 -p -q -w 80 -m 0 -k 500 -f -8 -g -2 -E 1000 -s BL62 "%s" "%s"' %(subject_file.name,shuffled_target_file.name)
        ssearch = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
