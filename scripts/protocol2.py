...
class Compare:
    ...
    def generate_report(self):
        ...
        tblComment = ( "# Subject: " + self.subject_name
                       + " Target: " + self.target_name)
        tblLines  = [tblComment]
        listHead = ["Subject ID","Target ID","SS Z-Score","GSAT Z-Score",
                    "Subject align-length","Target align-length",
                    "Subject seq","Target seq","TMO-score"]
        ...
        for result in self.globaldata:
            if self.srestrict is not None and self.trestrict is not None:
                if ( int(result['subject_tmc']) != self.srestrict
        ...
        with open(self.outdir+'/report.html','wb+') as output:
            output.write(template)
        ##### print tab-separated table
        with open(self.outdir+'/report.tbl','wb+') as outputtab:
            for i in tblLines:
...
'''
    ...
    opts.add_option('-o',
                    ...
                    default='protocol2',
                    help='Path to output')
    opts.add_option('--subject',
                    action='store',
                    dest='sname',
                    type='string',
                    default=None,
                    help='Subject Name to appear on report')
    opts.add_option('--target',
                    action='store',
                    dest='tname',
                    type='string',
                    default=None,
                    help='Target Name to appear on report')
    opts.add_option(
        '--assign',
        action='store',
        dest='num',
        type='int',
        default=3,
        help='TSS Setting :: Number of targets to assign each subject')
    opts.add_option('--shuffle',
                    action='store',
                    dest='rand',
                    type='int',
                    default=300,
                    help='Number of times to shuffle each alignment with GSAT')
    opts.add_option(
        '--stms',
        action='store',
        dest='srestrict',
        type='int',
        default=None,
        help='Report will contain X TMSs subjects. TTMS must be set to work')
    opts.add_option(
        '--ttms',
        action='store',
        dest='trestrict',
        type='int',
        default=None,
        help='Report will contain X TMSs targets. STMS must be set to work')
    opts.add_option('--min',
                    action='store',
                    dest='minAln',
                    default=60,
    ...
    if ( os.path.exists(cli.subject) is False
